...
#
# Description: Creates Figures 2i and 2j in Sloan and Feng (2023)
################################################################################
...
Slp.ldfxn <- function(ID, flt) {
  load(
    paste0(
      "./03-outputs/02-robust-framework/01-stress-perf-dist/",
      ID,
      "_SlpPrfDist.RData"
    )
  )
...
plot(gps1)
ggsave2(paste0(svpath,"fig-02i.pdf"))
...
plot(gxy)
ggsave2(paste0(svpath,"fig-02j-1.pdf"))

# The x-axis marginal plot
...
geom_vline(xintercept = 0.4, lwd = lw, linetype = "dashed") + 
  force_panelsizes(cols = unit(f.wd, "in"), rows = unit(f.wd/ f.ar, "in"))
plot(gx)
ggsave2(paste0(svpath,"fig-02j-2.pdf"))

# The y-axis marginal plot
...
plot(gy)
ggsave2(paste0(svpath,"fig-02j-3.pdf"))

# Could do all in one, but I wanted more flexibility
# wrap_plots(
#   gx, 
#   plot_spacer(), 
#   gxy,
