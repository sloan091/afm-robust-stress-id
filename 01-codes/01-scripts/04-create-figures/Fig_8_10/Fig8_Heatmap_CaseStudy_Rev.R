#===============================================================================
# Name: Fig7_Robust_Heatmaps
# By: Brandon Sloan
# Last Updated: 1/18/23
# Description: This script takes the robustness metrics and 
# plots them on a heatmap with the median stress signal and median performance
# metrics.  Ultimately, I categorize each site as robust
# wet, dry or no stress.
#===============================================================================
...
# Select Sites
SiteSel <- c("US_Me1","US_Me2","US_Me3","US_Me5","US_Me6","CA_SF2")
...
#save(RbSlpCat,file = "./Data/FLX_Factorial_Robustness_Categories.RData")
#tst <- RbSlpCat$data[[32]]
...
htPerf.2 <- htPerf.1 %>% group_by(SiteID) %>%
  summarize(RbStr.n = n_distinct(RbStrID),
            Str.n = n_distinct(StrID)) %>% ungroup() %>%
  left_join(htPerf.1, by = "SiteID") %>% ungroup() %>%
  group_by(SiteID, IGBP, RbStrID) %>%
  summarize(
    n = n(),
    RbStr.n = unique(RbStr.n),
...
htPerf <- htPerf.2 %>% group_by(SiteID,IGBP) %>% nest() %>% 
  ungroup() %>%
  mutate(RbStr = map(data,  ~ grbfxn.prf(.x,perfVars)))  %>%
  unnest(RbStr) %>% select(-data,-id) %>%
  mutate(
    across(contains("_sd"),~1 - (.))) %>%
  pivot_longer(cols = perfVars,
               names_to = "mtcID",
               values_to = "MtcVal") %>%
  mutate(mtcID = fct_relevel(mtcID, all_of(perfVars))) %>%
...
#===================================
pltDat <- RbSlpCat %>% filter(SiteID %in% SiteSel) %>%
...
g1 <-
  g1 + geom_point(aes(shape = RbID, size = RbID),
                  color = "black",
                  stroke = stk) +
  scale_shape_manual(
    name = "Col. 1: Robust Class",
    values = c(Both = 10, Prf = 3, Slp = 1),
    labels = c(Both = "Stress + Performance", Prf = "Performance", Slp = "Stress"),
    drop = FALSE
  ) +
  scale_size_manual(
    name = "Col. 1: Robust Class",
...
# B. Normalized slope plot
#=========================
pltDat <-
  RbSlp %>% filter(SiteID %in% SiteSel & !(SlpID == 3 &
                                        Slp_nrm < 0)) %>%
...
#================================
selVars = c("LCE_Rob","AfT1_Rob","AfT2_Rob","AfT3_Rob")
pltDat <- htPerf %>% filter(SiteID %in% SiteSel & mtcID %in% selVars) %>%
...
g3 <- pltDat %>%
  htHelp("mtcID",
         ...
         "") +
  scale_fill_gradientn(
    colors = rev(hcl.colors(20, "Greens3")),
    breaks = c(0, 1),
    limits = c(0, 1),
    oob = scales::squish
  ) +
...
gall <- (g1 + g2 + g3) + plot_layout(ncol = 3)

# Save figure
ggsave2(paste0("./Figs/AFM/Revision/Fig_8_10/Fig8",v,".pdf"),width = 6,height = 3, units = "in")
